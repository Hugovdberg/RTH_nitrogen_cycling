...
# Define helper functions for solving ----
...
plot_steady <- function(filename,...) {
    ...
    obs[, "O2"] <- dissolvedO2
    obs[, "NH3"] <- amonium
    plot(std, obs = obs, grid = grid$x.mid, ylab = 'Concentration [umol/L]', xlab = 'Distance [m]')

    rates <- cbind(std$Raero, std$Rdenit, std$Rnit, std$Raeration, std$Rdgas)
    matplot(grid$x.mid, rates, type = 'l', xlab = 'Distance [m]', ylab = 'Rate [umol/h]')
    legend('topright',
           c('Aerob. resp.', 'Denitrification', 'Nitrification', 'Aeration O2', 'Aeration N2'),
           lty = 1:5,
           col = 1:5)
    dev.copy2pdf(file = filename)
    return(invisible(std))
}

# Solve and plot steady state variations ----
# plotting model
plot_steady(filename = 'output/steady_init_param.pdf')
std <- plot_steady(filename = 'output/steady_fit.pdf',
                   k1 = 2e-3, k2 = 3.5e-4, k3 = 3e-4)
plot_steady(filename = 'output/steady_inc_DOM.pdf',
            k1 = 2e-3, k2 = 3.5e-4, k3 = 3e-4,
            DOMin = 6)
plot_steady(filename = 'output/steady_dec_DOM.pdf',
            k1 = 2e-3, k2 = 3.5e-4, k3 = 3e-4,
            DOMin = 2)
plot_steady(filename = 'output/steady_inc_aeration.pdf',
            k1 = 2e-3, k2 = 3.5e-4, k3 = 12e-4)
plot_steady(filename = 'output/steady_dec_aeration.pdf',
            k1 = 2e-3, k2 = 3.5e-4, k3 = 0)
plot_steady(filename = 'output/steady_inc_nitrification.pdf',
            k1 = 2e-3, k2 = 3.5e-4*2, k3 = 3e-4)
plot_steady(filename = 'output/steady_dec_nitrification.pdf',
            k1 = 2e-3, k2 = 3.5e-4*0, k3 = 3e-4)
plot_steady(filename = 'output/steady_inc_resp.pdf',
            k1 = 2e-3*2, k2 = 3.5e-4, k3 = 3e-4)
plot_steady(filename = 'output/steady_dec_resp.pdf',
            k1 = 2e-3*0.5, k2 = 3.5e-4, k3 = 3e-4)
...
# Solve and plot transient model with real life inputs ----

total_time = 24*365*8.01 # 8 years in hours
step_size = 24*14
times <- seq(0, total_time, by = step_size)
out <- solve_dom_model_transient(times, k1 = 2e-3, k2 = 3.5e-4, k3 = 3e-4)
out[,'time'] <- out[,'time']/24/365
image(out, legend = TRUE, grid = grid$x.mid, col = viridis(256), xlab = 'Time [yr]', ylab = 'Distance [yr]')
